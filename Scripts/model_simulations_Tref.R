...
tgrid <- seq(15,30, length = 5) + 272.15
...
singlerun <- function(idx){
  ...
  if(idx <= 5){
    paramscur[32] = tgrid[idx]
  }else{
    paramscur[33] = tgrid[(idx-5)]
  }
  ...
  if(ERRRRR) print(paste("Warning ", idx, " not stable"))
  ...
  out3 <- cbind(out2,
                PARS = c(unname(paramscur), rep(-2, lg - length(paramscur))),
                YSTABLE = c(unname(ystable2), rep(-2, lg - length(ystable2))))
  
  print(paste("Done", idx))
  return(out3)
...
results = tibble(data.frame(outf)) %>% 
  select(Run, PARS) %>%
  filter(PARS > 100 & PARS <400) %>%
  mutate(PARM = rep(c("Trefa", "Trefp"), 10)) %>%
  pivot_wider(names_from = PARM, values_from = PARS) %>%
  right_join(
    tibble(data.frame(outf)) %>%
      filter(time == 1395 & Treatment <=3) %>%
      select(P:Run), by = "Run"
  ) %>%
  select(-Expt) %>%
  pivot_longer(P:H,names_to = "StateVar") %>%
  mutate(Treatment = paste0("X", Treatment)) %>%
  pivot_wider(names_from = Treatment) %>%
  mutate(IE = X3 - X2-X1 +X0,
         HE = X2 - X0,
         WE = X1 - X0)

results %>%
  select(Run,StateVar,Trefa, Trefp,IE, HE, WE) %>%
  filter(Trefp == 297.65) %>%
  filter(StateVar %in% c("N", "P", "S", "M", "L")) %>%
  pivot_longer(IE:WE,names_to = "Effect") %>%
  ggplot(aes(x = Trefa, y = value, color = Effect, group = Effect)) + facet_wrap(.~StateVar, scales = "free") + geom_line() + theme_classic() + xlab("Tref,W")
...
results %>%
  ...
  filter(StateVar %in% c("N", "P", "S", "M", "L")) %>%
  pivot_longer(IE:WE,names_to = "Effect") %>%
  ggplot(aes(x = Trefp, y = value, color = Effect, group = Effect)) + facet_wrap(.~StateVar, scales = "free") + geom_line() + theme_classic() + xlab("Tref,P")
